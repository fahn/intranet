...
class Tools 
{
    ...
    protected string $iniFile = BASE_DIR .'/inc/config.ini';
     ...
     * @return array
     */
    public static function escapeInput(array $data): ?array
    {
        if (is_array($data)) 
        {
            return $data;
    ...
    public function customRedirect(array $url): void
    {
        try {
     ...
     * @return void
     */
    public function getIniValue($val = ""): ?array
    {
        $ini = self::getIni();
    ...
    {
        $modes = $this->getIniValue("tournament")["types"];
        if (is_array($modes)) 
        {
            asort($modes);
        }
    ...
    public function infoLog(string $message): string
    {
    ...
    {
        return (int) $this->getIniValue("HomeClubId");
    }
