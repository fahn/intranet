...
class PrgPatternElementRanking extends APrgPatternElement
{
    ...
    private function insertMatch(): bool
    {
        ...
        $sets = array();
        try {
            $set1 = strval(trim(implode(":", $this->getAllPostVariable(self::FORM_ITEM_SET1))));
            $sets[] = $set1;

            $set2 = strval(trim(implode(":", $this->getAllPostVariable(self::FORM_ITEM_SET2))));
            $sets[] = $set2;
       
            $set3Arr = $this->getAllPostVariable(self::FORM_ITEM_SET3);
    ...
    {
        $loss = ($win == 1 ? 0 : 1);
        if ($this->db->updatePoints($playerId, $points, $win, $loss)) {
            return true;
        }
        return false;
    }
